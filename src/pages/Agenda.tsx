...
interface Dentista { id: string; nome: string; dias_semana: string[]; }
...
export default function Agenda() {
  ...
  useEffect(() => {
    ...
    supabase.from("dentista_clinica").select("dias_semana, dentistas(id, nome)").eq("clinica_id", clinicaSelecionada).then(({ data }) => {
      if (data) {
  ...
  useEffect(() => {
    ...
    async function carregar() {
      ...
      let query = supabase
        .from("agendamentos")
        .select(`id, paciente_id, dentista_id, clinica_id, procedimento_id, data_hora, status, observacoes, pacientes(nome), procedimentos(nome)`)
        .eq("clinica_id", clinicaSelecionada)
        .gte("data_hora", inicio)
        .lte("data_hora", fim)
        .order("data_hora");

      if (dentistaSelecionado !== "todos") query = query.eq("dentista_id", dentistaSelecionado);
  ...
  function recarregar() { setSemanaBase(new Date(semanaBase.getTime())); }
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 -m-6">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1 ml-2">
              <button onClick={() => setSemanaBase(addDays(semanaBase, -7))} className="p-1.5 rounded hover:bg-gray-100 text-gray-500"><ChevronLeft size={16} /></button>
              <span className="text-sm font-medium text-gray-700 w-48 text-center">{formatWeekRange(semanaBase)}</span>
              <button onClick={() => setSemanaBase(addDays(semanaBase, 7))} className="p-1.5 rounded hover:bg-gray-100 text-gray-500"><ChevronRight size={16} /></button>
              <button onClick={() => setSemanaBase(getMondayOfWeek(new Date()))} className="ml-1 px-3 py-1 text-xs font-medium bg-blue-50 text-blue-700 rounded hover:bg-blue-100">Hoje</button>
            </div>
          </div>
      ...
      <div className="flex-1 overflow-auto px-2 py-3">
        {loading ? (
          <div className="flex items-center justify-center h-64 text-gray-400 text-sm">Carregando agenda...</div>
        ) : (
                    ...
                    return (
                      <th key={dia.key} className="pb-2 text-center">
                        <div className="flex flex-col items-center gap-0.5">
                          <span className={`text-[11px] uppercase tracking-wide font-medium ${disp ? "text-gray-500" : "text-gray-300"}`}>{dia.label}</span>
                ...
                {HORARIOS.map(hora => (
                  <tr key={hora}>
                    ...
                    {diasComData.map(dia => {
                      ...
                      return (
                        <td key={dia.key}
                          onClick={() => { if (!disp || almoco) return; setCelulaSelecionada({ data: dia.data, hora }); setModalAberto(true); }}
                          className={`border-t border-gray-100 align-top p-0.5 h-9 min-h-[36px] ${almoco ? "bg-amber-50" : ""} ${!disp && !almoco ? "bg-gray-50/80" : ""} ${isHoje && disp && !almoco ? "bg-blue-50/20" : ""} ${disp && !almoco ? "cursor-pointer hover:bg-blue-50 transition-colors" : ""}`}>
                          {almoco && hora === "12:00" && <span className="text-[10px] text-amber-500 font-medium px-1">Almoço</span>}
                          {agds.map(ag => {
...
function ModalNovoAgendamento({ clinicaId, dentistas, celula, dentistaPre, onClose, onSalvo }: {
  clinicaId: string; dentistas: Dentista[]; celula: { data: string; hora: string } | null;
  dentistaPre: string; onClose: () => void; onSalvo: () => void;
}) {
  ...
  async function salvar() {
    ...
    setSalvando(true);
    // Monta o datetime no fuso de São Paulo corretamente
    const dataHora = `${data}T${hora}:00-03:00`;
    await supabase.from("agendamentos").insert({
      paciente_id: pacienteId, dentista_id: dentistaId, clinica_id: clinicaId,
      procedimento_id: procedimentoId || null, data_hora: dataHora, observacoes, status: "agendado",
    });
    setSalvando(false);
    onSalvo();
  }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="font-semibold text-gray-800 flex items-center gap-2"><Plus size={17} className="text-blue-600" /> Novo Agendamento</h2>
...
function ModalDetalhe({ agendamento, onClose, onAtualizado }: {
  agendamento: Agendamento; onClose: () => void; onAtualizado: () => void;
}) {
  const [status, setStatus] = useState(agendamento.status);
  const [salvando, setSalvando] = useState(false);
  const cfg = STATUS_CONFIG[agendamento.status] ?? STATUS_CONFIG.agendado;
  const dataFormatada = new Date(agendamento.data_hora).toLocaleDateString("pt-BR", { timeZone: TZ });
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="font-semibold text-gray-800 flex items-center gap-2"><User size={17} className="text-blue-600" /> Agendamento</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600"><X size={19} /></button>
        </div>
        <div className="px-6 py-4 flex flex-col gap-3">
          <div><p className="text-xs text-gray-400">Paciente</p><p className="font-semibold text-gray-800">{agendamento.paciente_nome}</p></div>
          <div><p className="text-xs text-gray-400">Procedimento</p><p className="text-gray-700">{agendamento.procedimento_nome}</p></div>
          <div className="flex gap-6">
            <div><p className="text-xs text-gray-400">Data</p><p className="text-gray-700">{dataFormatada}</p></div>
            <div><p className="text-xs text-gray-400">Horário</p><p className="text-gray-700">{horaFormatada}</p></div>
          </div>
          {agendamento.observacoes && <div><p className="text-xs text-gray-400">Observações</p><p className="text-gray-600 text-sm">{agendamento.observacoes}</p></div>}
          <div><p className="text-xs text-gray-400 mb-1">Status atual</p><span className={`text-xs px-2 py-0.5 rounded border font-medium ${cfg.cor}`}>{cfg.label}</span></div>
          <div>
